...
def create_models(encoded_size, T, use_residual):
  ...
  decoder_counter = tf.keras.Sequential(name="decoder")
  for _ in range(FLAGS.decoder_layers-1):
...
def get_train_model(model, discriminator, optimizer, datas, discriminator_opt, T, reg_amount=0):
  @tf.function
  def train_step(batch, batch_targets, adver_batch, decoder, decoder_counter, indexes_to_train, indexes_to_adver, train_model):
  ...
  def train_model(decoder, decoder_counter, indexes_to_train, indexes_to_adver, train_model, stop_acc, metric_prefix):
    ...
    for step_i in range(FLAGS.max_train_steps):
      ...
      train_step(tf.constant(batch), tf.constant(targets), tf.constant(adver_batch), decoder, decoder_counter, indexes_to_train, indexes_to_adver, train_model)
      if step_i % FLAGS.eval_interval == 0:
        ...
        if train_mse_metric.result().numpy() < stop_acc and step_i > 0:
...
def main(_):
  ...
  train_acc = get_train_model(model, discriminator, optimizer, datas, discriminator_opt, FLAGS.num_timesteps, reg_amount=FLAGS.reg_amount)(
    decoder, decoder_counter, train_indexies, [], True, target_train_accuracy, "train_full_model")
